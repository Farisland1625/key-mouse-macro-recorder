...
class MacroFormatTests(unittest.TestCase):
    def test_repeat_settings_accept_arbitrary_counts_and_loop_mode(self):
        self.assertEqual(macro_recorder.parse_repeat_settings(macro_recorder.REPEAT_MODE_COUNT, "100000000000000000000"), 100000000000000000000)
        self.assertIsNone(macro_recorder.parse_repeat_settings(macro_recorder.REPEAT_MODE_LOOP, ""))
        with self.assertRaisesRegex(ValueError, "重复次数"):
            macro_recorder.parse_repeat_settings(macro_recorder.REPEAT_MODE_COUNT, "0")
        with self.assertRaisesRegex(ValueError, "重复模式"):
            macro_recorder.parse_repeat_settings("other", "1")

    def test_loop_progress_text_is_unbounded(self):
        self.assertEqual(
            macro_recorder.MacroApp._format_playback_progress(0.5, 1.0, 1.0, loop_index=4, repeat=None),
            "第 5 次 · 循环 · 0.50s / 1.00s",
        )

    def test_loop_worker_stops_only_after_stop_event(self):
        app = macro_recorder.MacroApp.__new__(macro_recorder.MacroApp)
    ...
    def test_hook_source_timestamp_handles_32_bit_rollover(self):
        ...
        source_tick = 0x10
        self.assertEqual(
            macro_recorder.expand_hook_tick(source_tick, reference_tick),
            (8 << 32) + source_tick,
        )
        self.assertAlmostEqual(
            macro_recorder.hook_elapsed_seconds(source_tick, start_tick, reference_tick),
            0.032,
        )

    def test_recording_hook_uses_one_thread_for_keyboard_and_mouse(self):
        hook = macro_recorder.RecordingHookThread(mock.Mock(), mock.Mock(), mock.Mock())
        self.assertEqual(
            set(hook.handlers),
            {macro_recorder.WH_KEYBOARD_LL, macro_recorder.WH_MOUSE_LL},
        )
    ...
    def test_recording_stop_sorts_source_times_and_preserves_equal_time_order(self):
        ...
        app.events = [
            ...
            {"kind": "mouse", "t": 0.009, "message": macro_recorder.WM_LBUTTONDOWN},
    ...
    def test_events_are_normalized_and_monotonic(self):
        events = macro_recorder.validate_events([
    ...
    def test_move_event_keeps_slots_ordered_and_preserves_time_gaps(self):
        ...
        moved = macro_recorder.move_event(events, 1, -1)
    ...
    def test_non_finite_timestamps_and_delay_events_are_rejected(self):
        with self.assertRaisesRegex(ValueError, "timestamp"):
            macro_recorder.validate_events([{"kind": "key", "t": "nan", "vk": 65}])
        with self.assertRaisesRegex(ValueError, "unknown kind"):
            macro_recorder.validate_events([{"kind": "delay", "t": 0, "duration": "inf"}])
        with self.assertRaises(ValueError):
            macro_recorder.build_action_events("空白", base_time=1.0)

    def test_event_code_ranges_and_boolean_values_are_normalized_strictly(self):
        normalized = macro_recorder.validate_events([
            {"kind": "key", "t": 0, "vk": 65, "scan": 30, "action": "down", "extended": "false"},
        ])
        self.assertFalse(normalized[0]["extended"])
        with self.assertRaises(ValueError):
            macro_recorder.validate_events([{"kind": "key", "t": 0, "vk": 65, "scan": -1}])
        with self.assertRaises(ValueError):
            macro_recorder.validate_events([{"kind": "key", "t": 0, "vk": 65, "extended": "maybe"}])
        with self.assertRaises(ValueError):
            macro_recorder.validate_events([{"kind": "mouse", "t": 0, "message": 999}])
        with self.assertRaises(ValueError):
            macro_recorder.validate_events([{"kind": "mouse", "t": 0, "x": 1 << 40}])

    def test_delay_module_is_rejected(self):
        with self.assertRaisesRegex(ValueError, "unknown kind"):
            macro_recorder.validate_events([{"kind": "delay", "t": 0.25, "duration": "0.25"}])

    def test_key_display_names_hide_codes_from_user(self):
        self.assertEqual(macro_recorder.key_display_name(65), "A")
        self.assertEqual(macro_recorder.key_display_name(46), "Delete")
        self.assertEqual(macro_recorder.key_display_name(8), "Backspace")
        self.assertEqual(macro_recorder.key_display_name(46, "Delete"), "Delete")
        self.assertEqual(macro_recorder.key_display_name(8, "BackSpace"), "Backspace")
        self.assertEqual(macro_recorder.key_display_name(65, "??"), "A")
        self.assertEqual(macro_recorder.key_display_name(65, "a"), "A")
        self.assertEqual(macro_recorder.tk_event_vk(SimpleNamespace(keycode=229, char="a", keysym_num=97)), 65)
    ...
    def test_mouse_action_labels_include_wheel_direction_and_side_button(self):
        up = (120 & 0xFFFF) << 16
        down = ((-120) & 0xFFFF) << 16
        self.assertEqual(macro_recorder.mouse_action_label(macro_recorder.WM_MOUSEWHEEL, up), "垂直滚轮向上")
        self.assertEqual(macro_recorder.mouse_action_label(macro_recorder.WM_MOUSEWHEEL, down), "垂直滚轮向下")
        self.assertEqual(macro_recorder.mouse_action_label(macro_recorder.WM_MOUSEHWHEEL, up), "水平滚轮向右")
        self.assertEqual(macro_recorder.mouse_action_label(macro_recorder.WM_MOUSEHWHEEL, down), "水平滚轮向左")
        self.assertEqual(macro_recorder.mouse_action_label(macro_recorder.WM_XBUTTONDOWN, 1 << 16), "侧键1按下")
        self.assertEqual(macro_recorder.mouse_action_label(macro_recorder.WM_XBUTTONUP, 2 << 16), "侧键2释放")
        self.assertEqual(macro_recorder.wheel_amount(down), 120)

    def test_shared_action_builder_supports_insert_and_property_modes(self):
        inserted_down = macro_recorder.build_action_events("左键按下", base_time=1.0, x=10, y=20)
        self.assertEqual(len(inserted_down), 1)
        self.assertEqual(inserted_down[0]["message"], macro_recorder.WM_LBUTTONDOWN)

        edited_wheel = macro_recorder.build_action_events("垂直滚轮向下", base_time=2.0, delta=240)
        self.assertEqual(len(edited_wheel), 1)
        self.assertEqual(edited_wheel[0]["t"], 2.0)
        self.assertEqual(macro_recorder.signed_word(macro_recorder.high_word(edited_wheel[0]["data"])), -240)

        with self.assertRaises(ValueError):
            macro_recorder.build_action_events("左键点击", base_time=1.0)

    def test_action_catalog_is_single_step_and_shared(self):
        labels = [entry["label"] for entry in macro_recorder.ACTION_CATALOG]
        self.assertEqual(list(macro_recorder.EVENT_TYPE_VALUES), ["鼠标", "键盘"])
        self.assertEqual(
            [entry["label"] for entry in macro_recorder.ACTION_CATALOG if entry["kind"] == "mouse"],
            list(macro_recorder.MOUSE_ACTION_VALUES),
        )
        self.assertEqual(macro_recorder.ACTION_TYPE_VALUES["键盘"], ("按下", "释放"))
        self.assertEqual(macro_recorder.ACTION_TYPE_VALUES["鼠标"], macro_recorder.MOUSE_ACTION_VALUES)
        self.assertFalse(any("点击" in label for label in labels))
        for label in labels:
            if label != "键盘按键":
                created = macro_recorder.build_action_events(label, base_time=1.0, x=10, y=20, delta=120)
                self.assertEqual(len(created), 1, label)

    def test_event_category_and_action_type_labels(self):
        key = {"kind": "key", "action": "up"}
        mouse = {"kind": "mouse", "message": macro_recorder.WM_MOUSEWHEEL, "data": ((120 & 0xFFFF) << 16)}
        self.assertEqual(macro_recorder.event_category_for_event(key), "键盘")
        self.assertEqual(macro_recorder.action_type_for_event(key), "释放")
        self.assertEqual(macro_recorder.event_category_for_event(mouse), "鼠标")
        self.assertEqual(macro_recorder.action_type_for_event(mouse), "垂直滚轮向上")
        self.assertEqual(
            macro_recorder.action_type_for_event({"kind": "mouse", "message": macro_recorder.WM_MOUSEMOVE}),
            "移动",
        )
        with self.assertRaisesRegex(ValueError, "错误的事件类型"):
            macro_recorder.validate_editor_choice("空白", "空白")

    def test_editor_choices_accept_text_and_reject_unknown_values(self):
        self.assertEqual(macro_recorder.validate_editor_choice(" 鼠标 ", " 左键按下 "), ("鼠标", "左键按下"))
        self.assertEqual(macro_recorder.validate_editor_choice("键盘", "释放"), ("键盘", "释放"))
        with self.assertRaisesRegex(ValueError, "错误的事件类型"):
            macro_recorder.validate_editor_choice("视频", "按下")
        with self.assertRaisesRegex(ValueError, "错误的动作"):
            macro_recorder.validate_editor_choice("鼠标", "点击")

    def test_timeline_gaps_represent_waits(self):
        events = macro_recorder.validate_events([
            {"kind": "key", "t": 1.0, "vk": 65, "action": "down"},
            {"kind": "mouse", "t": 3.5, "message": macro_recorder.WM_MOUSEMOVE, "x": 0, "y": 0},
        ])
        self.assertEqual(macro_recorder.event_end_time(events[0]), 1.0)
        self.assertEqual(max(macro_recorder.event_end_time(event) for event in events), 3.5)
    ...
    def test_release_inputs_retries_and_keeps_only_failed_states(self):
        ...
        active_buttons = {(macro_recorder.MOUSEEVENTF_LEFTUP, 0)}
        with (
            mock.patch.object(
                macro_recorder,
                "send_keyboard",
            ...
            mock.patch.object(macro_recorder.time, "sleep"),
        ):
            failures = app._release_inputs(active_keys, active_buttons)
        self.assertEqual(send_keyboard.call_count, 2)
        self.assertEqual(send_mouse.call_count, macro_recorder.INPUT_RELEASE_ATTEMPTS)
        self.assertEqual(active_keys, set())
        self.assertEqual(active_buttons, {(macro_recorder.MOUSEEVENTF_LEFTUP, 0)})
    ...
    def test_atomic_payload_round_trip(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            macro_recorder.save_macro_payload(path, [{"kind": "mouse", "t": 0, "message": macro_recorder.WM_MOUSEMOVE, "x": 2, "y": 3, "data": 0}])
            payload, events = macro_recorder.load_macro_payload(path)
    ...
    def test_atomic_payload_cleans_temporary_file_when_replace_fails(self):
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "macro.json"
            with mock.patch.object(macro_recorder.os, "replace", side_effect=OSError("replace failed")):
                with self.assertRaisesRegex(OSError, "replace failed"):
                    macro_recorder.save_macro_payload(
                        path,
                        [{"kind": "key", "t": 0, "vk": 65, "action": "down"}],
    ...
    def test_macro_load_requires_current_schema_version(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaises(ValueError):
                macro_recorder.load_macro_payload(path)
    ...
    def test_compose_macro_events_preserves_sequence_repeats_and_gaps(self):
        ...
        composed = macro_recorder.compose_macro_events([
    ...
    def test_compose_macro_events_maps_each_source_screen_to_output_screen(self):
        composed = macro_recorder.compose_macro_events(
            [{
                "events": [{"kind": "mouse", "t": 0, "message": macro_recorder.WM_MOUSEMOVE, "x": 99, "y": 49}],
                "repeat": 1,
    ...
    def test_compose_macro_events_rejects_empty_items_and_invalid_repeats(self):
        with self.assertRaisesRegex(ValueError, "至少需要一个宏"):
            macro_recorder.compose_macro_events([])
        with self.assertRaisesRegex(ValueError, "没有可编排事件"):
            macro_recorder.compose_macro_events([{"events": [], "repeat": 1}])
    ...
    def test_composed_payload_round_trip_leaves_source_files_unchanged(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            payload, events = macro_recorder.load_macro_payload(source)
            composed = macro_recorder.compose_macro_events([
                {"events": events, "repeat": 3, "screen": payload["screen"]}
            ])
            macro_recorder.save_macro_payload(output, composed, screen_bounds=payload["screen"])
            _output_payload, output_events = macro_recorder.load_macro_payload(output)
    ...
    def test_screen_point_mapping(self):
        ...
        with mock.patch.object(macro_recorder, "virtual_screen_bounds", return_value=source):
            mapped = macro_recorder.map_screen_point(960, 540, source)
    ...
    def test_clamp_screen_point_keeps_negative_hook_coordinates_reachable(self):
        ...
        self.assertEqual(macro_recorder.clamp_screen_point(1525, -6, bounds), (1525, 0))
        self.assertEqual(macro_recorder.clamp_screen_point(-20, 1200, bounds), (0, 1066))
    ...
    def test_clamp_screen_point_preserves_negative_secondary_monitor_coordinates(self):
        ...
        self.assertEqual(macro_recorder.clamp_screen_point(-100, -50, bounds), (-100, -50))

    def test_delete_mouse_moves_removes_every_move_and_preserves_actions(self):
        events = [
            {"kind": "mouse", "t": 0.0, "message": macro_recorder.WM_MOUSEMOVE, "x": 1, "y": 1},
            {"kind": "mouse", "t": 0.1, "message": macro_recorder.WM_MOUSEMOVE, "x": 2, "y": 2},
            {"kind": "mouse", "t": 0.2, "message": macro_recorder.WM_MOUSEMOVE, "x": 3, "y": 3},
            {"kind": "key", "t": 0.3, "vk": 65, "action": "down"},
            {"kind": "mouse", "t": 0.4, "message": macro_recorder.WM_MOUSEMOVE, "x": 4, "y": 4},
            {"kind": "mouse", "t": 0.5, "message": macro_recorder.WM_MOUSEMOVE, "x": 5, "y": 5},
            {"kind": "mouse", "t": 0.6, "message": macro_recorder.WM_MOUSEMOVE, "x": 6, "y": 6},
            {"kind": "mouse", "t": 0.7, "message": macro_recorder.WM_LBUTTONDOWN, "x": 7, "y": 7},
            {"kind": "mouse", "t": 0.8, "message": macro_recorder.WM_MOUSEMOVE, "x": 8, "y": 8},
            {"kind": "mouse", "t": 0.9, "message": macro_recorder.WM_MOUSEMOVE, "x": 9, "y": 9},
        ]
        filtered = macro_recorder.delete_mouse_moves(events)
        self.assertFalse(any(event.get("message") == macro_recorder.WM_MOUSEMOVE for event in filtered))
        self.assertEqual([event["kind"] for event in filtered], ["key", "mouse"])
        self.assertEqual(filtered[1]["message"], macro_recorder.WM_LBUTTONDOWN)

    def test_delete_mouse_moves_preserves_non_move_events_and_timestamps(self):
        events = [
            {"kind": "mouse", "t": 0.1, "message": macro_recorder.WM_MOUSEMOVE, "x": 1, "y": 2},
            {"kind": "key", "t": 0.2, "vk": 65, "action": "down"},
            {"kind": "mouse", "t": 0.3, "message": macro_recorder.WM_MOUSEMOVE, "x": 3, "y": 4},
            {"kind": "mouse", "t": 0.4, "message": macro_recorder.WM_MOUSEMOVE, "x": 5, "y": 6},
        ]
        filtered = macro_recorder.delete_mouse_moves(events)
    ...
    def test_delete_mouse_moves_removes_single_and_pure_move_traces(self):
        events = [
            {"kind": "mouse", "t": 0.1, "message": macro_recorder.WM_MOUSEMOVE, "x": 1, "y": 2},
            {"kind": "mouse", "t": 0.2, "message": macro_recorder.WM_MOUSEMOVE, "x": 3, "y": 4},
        ]
        self.assertEqual(macro_recorder.delete_mouse_moves(events), [])
        self.assertEqual(macro_recorder.delete_mouse_moves([events[0]]), [])
        self.assertEqual(macro_recorder.delete_mouse_moves([]), [])
